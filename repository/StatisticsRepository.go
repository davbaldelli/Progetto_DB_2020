...
func (s StatisticsRepository) GetBrandCarsUsage(brandName string) ([]models.CarUsage, error) {
	...
	if err := s.Db.Table("entries").Select("cars.*, count(entries.id) as 'usage'").Joins("right join cars ON entries.car = cars.id").Group("cars.id").Having("brand = ?", brandName).Scan(&statistics).Error; err != nil {
		return nil, err
	}
...
func (s StatisticsRepository) GetTrackLayoutsUsage(trackName string) ([]models.LayoutUsage, error) {
	...
	if err := s.Db.Table("races").Select("layouts.*", "count(races.id) as 'usage'").Joins("right join layouts on races.layout = layouts.id").Group("layouts.id").Having("track = ?", trackName).Scan(&statistics).Error; err != nil {
		return nil, err
	}
	return statistics, nil
}
